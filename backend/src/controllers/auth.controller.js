...
export async function login(req, res) {
  ...
  try {
    ...
    const token = jwt.sign({ id: user.id_usuario, categoria: user.categoria }, JWT_SECRET, { expiresIn: '1h' });

    res.json({
      token,
      user: { id: user.id_usuario, nombre: user.nombre, apellido: user.apellido, categoria: user.categoria },
    });
  } catch (err) {
...
export async function forgotPassword(req, res) {
  ...
  try {
    ...
    if (!rows.length) return res.status(200).json({ message: 'Si el email está registrado se enviará un enlace.' });

    const user = rows[0];
    const resetToken = jwt.sign({ id: user.id_usuario }, JWT_SECRET, { expiresIn: '1h' });

    await pool.query('UPDATE tokensrecuperacion SET usado=true WHERE usuario_id=$1 AND usado=false', [user.id_usuario]);
    await pool.query(
...
export async function resetPassword(req, res) {
  ...
  try {
    ...
    if (!rows.length) return res.status(400).json({ message: 'Enlace inválido o expirado' });

    const hash = await bcrypt.hash(newPassword, saltRounds);
    await pool.query('UPDATE usuarios SET contraseña=$1 WHERE id_usuario=$2', [hash, id]);
