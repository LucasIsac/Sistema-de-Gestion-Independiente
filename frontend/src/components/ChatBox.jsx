...
import PerfilUsuarioPanel from "./PerfilUsuarioPanel"; // 👈 Importamos el panel de perfil

const ChatBox = ({ receptor, userId }) => {
  const { mensajes, enviarMensaje } = useChat();
  const [texto, setTexto] = useState("");
  const [mostrarPerfil, setMostrarPerfil] = useState(false); // 👈 Estado para mostrar el perfil
  const chatEndRef = useRef(null);

  // 👉 Abrir y cerrar panel
  const handleVerPerfil = () => setMostrarPerfil(true);
  ...
  return (
    <>
      <div className="chat-box-wrapper">
        ...
        <div className="chat-header">
          <div className="chat-header-info">
            <h3 className="chat-partner-name clickable" onClick={handleVerPerfil}>
              {receptor.nombre}
            </h3>
      ...
      {/* 👇 Panel de perfil emergente */}
      {mostrarPerfil && (
        <PerfilUsuarioPanel usuario={receptor} onClose={handleCerrarPerfil} />
      )}
    </>
  );
};
